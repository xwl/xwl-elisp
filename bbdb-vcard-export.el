...
(defun xwl-bbdb-record-phones (record)
  (let ((cell-phone (bbdb-get-field record 'cell-phone))
        (home-phone (bbdb-get-field record 'home-phone))
        (ret '()))
    (unless (string= cell-phone "")
      (setq ret (list (list 'cell cell-phone))))
    (unless (string= home-phone "")
      (setq ret (cons (list 'home home-phone) ret)))
    ret))

(defun xwl-bbdb-phone-location (phone)
  (symbol-name (car phone)))

(defun xwl-bbdb-phone-string (phone)
  (cadr phone))
...
(defun bbdb-vcard-export-record-insert-vcard (record)
  ...
  (let ((name (bbdb-record-name record))
	...
	;; (phones (bbdb-record-phones record))
        (phones (xwl-bbdb-record-phones record))
    ...
    (dolist (phone phones)
      (insert "tel;type=" (xwl-bbdb-phone-location phone) ":"
              (xwl-bbdb-phone-string phone) "\n"))
